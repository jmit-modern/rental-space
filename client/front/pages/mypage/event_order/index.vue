<template>
  <div class="pt-5 max-w-7xl mx-auto">
    ...
    <mypage-layout>
      <div class="px-4 py-5">
        <div class="mb-8 text-xl">{{ $t('mypage.event_order.title') }}</div>
        <div class="pl-4 py-2 border-b-2 grid grid-cols-3 pr-16">
          <h1>{{ $t('mypage.event_order.ordererName') }}</h1>
          <h1 class="grid-col-span-2">
            {{ $t('mypage.event_order.eventName') }}
          </h1>
        </div>
        <div v-if="!$fetchState.pending && reservations" class="divide-y">
          ...
          >
            <div class="grid grid-cols-3 justify-between flex-1">
              <h1>
...
export default {
  ...
  async fetch() {
    ...
    await Promise.all([
      this.$store.dispatch('eventReservation/fetchOrders', query),
    ])
  },
  computed: {
    ...mapGetters({
      reservations: 'eventReservation/data',
    }),
    breadcrumbs() {
      const items = [
        {
