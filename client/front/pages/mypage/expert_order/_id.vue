<template>
  <div class="pt-5 max-w-7xl mx-auto">
    ...
    <mypage-layout>
      <div class="px-4 py-5">
        <div class="mb-8 text-xl">Expert Reservation Detail</div>
              ...
              <tr>
                ...
                <td class="w-3/4 border p-4">
                  <div class="flex items-center">
                    <span class="mr-2">{{ customer.username }}</span>
                  </div>
                </td>
              </tr>
              <tr>
                ...
                <td class="w-3/4 border p-4">
                  <div class="flex items-center">
                    <span class="mr-2">{{ reservation.location }}</span>
                  </div>
                </td>
              </tr>
              <tr>
                <th class="w-1/4 border p-4">
                  <div class="flex items-center space-x-6">
                    <outline-at-symbol-icon class="w-6 h-6 text-orange" />
                    <span>メールアドレス</span>
                  </div>
                </th>
                <td class="w-3/4 border p-4">
                  <div class="flex items-center">
                    <span class="mr-2">{{ customer.email }}</span>
                  </div>
                </td>
              </tr>
              <tr>
                <th class="w-1/4 border p-4">
                  <div class="flex items-center space-x-6">
                    <outline-device-mobile-icon class="w-6 h-6 text-orange" />
                    <span>電話番号</span>
                  </div>
                </th>
                <td class="w-3/4 border p-4">
        ...
        <div class="mb-8">
          ...
          <dl class="px-2 py-4 flex border-b">
            <dt class="w-1/5"></dt>
            <dd class="w-3/5 text-right">予約金額</dd>
        ...
        <div v-if="reservation.status == 'reserved'" class="mb-8">
          <h1 class="text-lg font-bold mb-4">専門家の詳細情報</h1>
          <div>
            <dl class="px-2 py-4 flex border-b">
            ...
            <dl class="px-2 py-4 flex border-b">
              ...
              <dd class="w-3/5">{{ customer.email }}</dd>
              <dd class="w-1/5 text-right"></dd>
            </dl>
            <dl class="px-2 py-4 flex border-b">
              <dt class="w-1/5">Phone number</dt>
              <dd class="w-3/5">{{ customer.phone }}</dd>
              <dd class="w-1/5 text-right"></dd>
            </dl>
          </div>
        </div>
...
export default {
  ...
  async fetch() {
    await Promise.all([
      this.$store.dispatch('expertReservation/fetchDetail', {
  ...
  computed: {
    ...mapGetters({
      reservation: 'expertReservation/detail',
    }),
    expert() {
      return this.reservation.expert || {}
    },
    reservationId() {
    ...
    breadcrumbs() {
      const items = [
        ...
        },
        {
          id: 3,
          name: 'エキスパート注文一覧',
          url: this.localeRoute({ name: 'mypage-expert_order' }),
        },
        {
          id: 4,
          name: '注文詳細',
  ...
  methods: {
    ...
    async approveOrder() {
      await this.$store.dispatch('expertReservation/approve', [
        this.reservationId,
      ])
    },
  },
