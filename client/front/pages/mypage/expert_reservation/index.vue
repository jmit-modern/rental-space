    ...
    <mypage-layout>
      <div class="px-4 py-5">
        <div class="mb-8 text-xl">
          {{ $t('mypage.expert_reservation.title') }}
        </div>
        <div class="pl-4 pr-16 py-2 grid grid-cols-4 border-b-2">
          <h1 class="col-span-3">エキスパートの名前</h1>
          <h1>予約時間</h1>
        </div>
        <div v-if="!$fetchState.pending && reservations" class="divide-y">
          ...
          >
            <div class="grid grid-cols-4 flex-1">
              <h1 class="col-span-3">
                {{ reservation.expert.user.firstName }}
                {{ reservation.expert.user.lastName }} <br />
                ({{ reservation.expert.skillTitle }})
              </h1>
              <span class="text-sm">
            ...
            <div class="flex-initial">
              <nuxt-link
                :to="
                  localePath({
                    name: 'mypage-expert_reservation-id',
...
export default {
  ...
  async fetch() {
    ...
    await Promise.all([this.$store.dispatch('expertReservation/fetch', query)])
  },
  computed: {
    ...mapGetters({
